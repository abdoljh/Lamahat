...
def _fit_to_frame(src: np.ndarray, w: int, h: int, *, is_depth: bool = False,
                  mismatch: float = 0.28) -> np.ndarray:
    """Place `src` into a w×h frame without amputating the subject.

    When the source aspect is close to the target, cover-crop as before. When it
    differs a lot (portrait-orientation photos, ultra-wide panoramas), *contain*
    the whole image and fill the margins — with a darkened, blurred copy of the
    image for colour layers, or with "far" depth for the depth layer. This is
    what stops tall character portraits from being cropped to their midsection
    when forced into a 16:9 frame.

    `mismatch` is the aspect tolerance for cover-fill.  At 0.28 the cover band is
    aspect ∈ [≈1.28, ≈2.28], so 3:2 (1.50) AND 4:3 (1.33) landscapes FILL the
    frame (≤~12.5%/side crop) rather than pillar-boxing — harmonising the "some
    shots fill, some have blurred bars" look.  Genuine portraits (ar < ~1.0)
    still fall to contain so the subject is preserved whole.  (Was 0.22, which
    pushed 4:3 sources just over the line into pillar-box.)
    """
    ih, iw = src.shape[:2]
    src_ar, tgt_ar = iw / ih, w / h
    if abs(src_ar - tgt_ar) / tgt_ar <= mismatch:
        return _cover_resize(src, w, h,
                             interp=cv2.INTER_CUBIC if not is_depth else cv2.INTER_CUBIC)
    # significant mismatch → contain the whole subject, pad the margins
    scale = min(w / iw, h / ih)
    nw, nh = max(1, int(round(iw * scale))), max(1, int(round(ih * scale)))
    fg = cv2.resize(src, (nw, nh),
                    interpolation=cv2.INTER_AREA if scale < 1 else cv2.INTER_CUBIC)
    x0, y0 = (w - nw) // 2, (h - nh) // 2
    if is_depth:
        canvas = np.zeros((h, w), np.float32)          # margins read as "far"
        canvas[y0:y0 + nh, x0:x0 + nw] = fg
    else:
        bg = _cover_resize(src, w, h, interp=cv2.INTER_CUBIC)
        bg = cv2.GaussianBlur(bg, (0, 0), max(w, h) * 0.02)
        canvas = (bg.astype(np.float32) * 0.80).astype(np.uint8)   # darken to pop the subject
        canvas[y0:y0 + nh, x0:x0 + nw] = fg
    return canvas
...
def render_parallax_clip(img_bgr: np.ndarray,
                         ...
                         *,
                         ...
                         pan_start: tuple[float, float] = (0.0, 0.0)) -> Path:
    ...
    zoom_start: baseline zoom the camera path STARTS from (default 1.0 = wide).
                The path's own zoom delta is added on top, so the move begins at
                ``zoom_start`` and eases further from there.  Used to continue a
                shot from a previous one's end framing (e.g. the typography-over-
                image background picking up exactly where the real shot left off)
                instead of snapping back to wide — which otherwise reads as a
                visible shrink at the cut.
    pan_start : baseline (cx, cy) the path starts from, added on top of the path's
                own offset — the lateral counterpart to ``zoom_start``.

    Continuity note: ``camera_path`` is smoothstep-eased (zero velocity at t=0),
    so a clip that starts from a prior clip's end (cx, cy, zoom) resumes from rest
    with no positional OR velocity jump; with the same image+depth+amp the seam
    ...
    # than crops) portrait / odd-aspect sources so the subject is never cut off.
    img = _fit_to_frame(img_bgr, bw, bh, is_depth=False)
    if depth.shape[:2] != img_bgr.shape[:2]:
        depth = cv2.resize(depth, (img_bgr.shape[1], img_bgr.shape[0]), interpolation=cv2.INTER_CUBIC)
    disp = _fit_to_frame(depth, bw, bh, is_depth=True)
    ...
    try:
        for f in range(n_frames):
            ...
            # Continue from a prior shot's end framing: the path's own deltas are
            # added on top of the start baseline.  At t=0 this is exactly
            # (pan_start, zoom_start); smoothstep makes it resume from rest.
            zoom = zoom_start + (zoom - 1.0)
            cx += pan_start[0]
            cy += pan_start[1]
            if warp == "inpaint":
...
def shot_end_camera(visual: str, **overrides) -> tuple[float, float, float]:
    """Return the (cx, cy, zoom) a parallax shot of ``visual`` finishes on.

    This is the camera state at the end of the path (t=1).  render.py uses it to
    start the typography-over-image background exactly where the preceding real
    shot ended, so the cut doesn't snap the framing back to wide.  ``overrides``
    mirror what render_shot_parallax accepts (e.g. a pinned motion/intensity).
    """
    p = params_for_visual(visual)
    p.update(overrides)
    return camera_path(p["motion"], 1.0, p["intensity"])
