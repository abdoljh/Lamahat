#!/usr/bin/env python3
"""
phase3_run.py — Standalone Phase 3 video generator (v2, shot-based).

Drives the shot-based `phase3/` package from the command line.  The pipeline
is: forced alignment → AI shot planner (one Sonnet call) → render.
...
Audio (optional — interpolated from character count when omitted)
------
...
Content context
---------------
    --book-title TEXT       Book title (improves shot/keyword quality)
    --character-name TEXT   Main character / subject name (Latin, for source search)
...
--------
    --anthropic-key KEY     ANTHROPIC_API_KEY — required for the planner +
                                                vision scoring
    --pexels-key KEY        PEXELS_API_KEY    — optional Pexels image source

Visual options
--------------
    --output PATH           Output .mp4  [default: output/phase3_video.mp4]
    --grade NAME            warm | cool | neutral | bw  [default: warm]
    --typography-family X   A | B | C  [default: A]
    --width N               [default: 1920]
    --height N              [default: 1080]
    --fps N                 [default: 25]
    --no-captions           Skip burning Arabic captions
    --no-vision             Disable Claude vision image scoring
    --book-cover PATH       Cover image for the title card
    --music PATH            Optional music bed mixed under the narration
    --book-extracts PATH    Phase 1a photos directory or ZIP (book imagery)

Modes
-----
    --dry-run               Parse sections + estimate durations, print, exit
    --align-only            Run forced alignment, print + save word timings, exit
    --plan-only             Run alignment + AI shot planner, save plan JSON, exit
                            (NO render)
...
Output extras
-------------
    --save-plan PATH        Write shot-plan JSON (always for --plan-only)
    --save-alignment PATH   Write word-timings JSON
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="phase3_run.py",
        description="Standalone Phase 3 video generator (v2, shot-based).",
    ...
    p.add_argument("--grade", default="warm",
                   choices=["warm", "cool", "neutral", "bw"])
    p.add_argument("--typography-family", default="A", choices=["A", "B", "C"])
    p.add_argument("--width", type=int, default=1920, metavar="N")
    p.add_argument("--height", type=int, default=1080, metavar="N")
    p.add_argument("--fps", type=int, default=25, metavar="N")
    ...
    p.add_argument("--align-only", action="store_true",
                   help="run forced alignment + print word timings, exit")
    p.add_argument("--plan-only", action="store_true",
                   help="run alignment + AI shot planner, save plan JSON, exit")
    ...
    p.add_argument("--save-plan", metavar="PATH",
                   ...
                   help="Path for the shot plan JSON [default: output/shot_plan.json]")
    p.add_argument("--save-alignment", metavar="PATH",
                   default="output/word_timings.json",
                   help="Path for the word-timings JSON [default: output/word_timings.json]")
...
def _print_plan(sections, durations) -> None:
    ...
    for sec, dur in zip(sections, durations):
        bar = int(dur / total * 40) if total else 0
...
def _resolve_audio_duration(audio_path: Path | None, script_text: str,
    ...
    if audio_path and audio_path.exists():
        from phase3 import probe_audio_duration
        dur = probe_audio_duration(audio_path)
        if dur > 0:
            return dur
    return min(360.0, max(60.0, len(script_text.strip()) / 12.0))


def _save_alignment(timings, path: Path) -> None:
...
def _run_align_only(args, script_text: str) -> None:
    ...
    if args.save_alignment:
        ...
        _save_alignment(timings, out_path)
...
def _run_plan_only(args, script_text: str, anthropic_key: str) -> None:
    ...
    print(f"  {len(shots)} shots planned in {time.perf_counter() - t0:.1f}s\n")
...
def main() -> int:
    ...
    # ── Full render (align → plan → render) ──────────────────────────── #
    if not anthropic_key:
        print("ERROR: full render requires an Anthropic API key "
              "(--anthropic-key or ANTHROPIC_API_KEY)", file=sys.stderr)
        return 1
    ...
    print(f"Render : {args.width}×{args.height} @ {args.fps} fps  "
          f"grade={args.grade}  family={args.typography_family}  "
          f"captions={'off' if args.no_captions else 'on'}")
    print(f"Claude : {'✓  (planner + vision)' if anthropic_key else '✗'}")
    print(f"Pexels : {'✓' if pexels_key else '✗  (other sources only)'}\n")
