...
def render_text_overlay(spec: "TypographySpec") -> Image.Image:
    """Transparent RGBA overlay = soft readability scrim + the shot's text.
    ...
    and centred, with a darkened band behind it so it reads over any background;
    everything outside the scrim/text is transparent.
    """
    import numpy as np
    ...
    # Scrim: a soft but clear horizontal plate centred on the text, plus a
    # mild global veil so bright/busy frames never wash the text out.  The band
    # is taller than the text block so the falloff is gentle (no hard bar edge).
    ys = np.arange(H, dtype=np.float32)
    sigma = max(115.0, total_h * 0.95 + main_size * 1.0)
    band = (np.exp(-((ys - center_y) / sigma) ** 2) * 205).astype(np.uint8)
    scrim = np.repeat(band[:, None], W, axis=1)
    veil = Image.new("RGBA", (W, H), (*CHARCOAL, 0))
    veil.putalpha(Image.fromarray(scrim, "L"))
    img.alpha_composite(Image.new("RGBA", (W, H), (*CHARCOAL, 40)))
    ...
    y = block_top
    for ln in lines:
        w_, _ = _measure(draw, ln, main_font)
        _draw_text_rtl(draw, ((W - w_) // 2, y), ln, font=main_font, fill=OFF_WHITE)
        y += lh
    ...
    if spec.subtitle:
        ...
        _draw_text_rtl(draw, ((W - sw) // 2, sy), sub_text, font=sub_font, fill=accent)
