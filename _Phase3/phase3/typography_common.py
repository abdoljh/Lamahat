...
def render_text_overlay(spec: "TypographySpec") -> Image.Image:
    ...
    # Scrim: a soft horizontal band of darkening centred on the text, plus a
    # faint global veil so bright frames never wash the text out.
    ys = np.arange(H, dtype=np.float32)
    sigma = max(60.0, total_h * 0.62 + main_size * 0.5)
    band = (np.exp(-((ys - center_y) / sigma) ** 2) * 165).astype(np.uint8)
    scrim = np.repeat(band[:, None], W, axis=1)
    veil = Image.new("RGBA", (W, H), (*CHARCOAL, 0))
    veil.putalpha(Image.fromarray(scrim, "L"))
    img.alpha_composite(Image.new("RGBA", (W, H), (*CHARCOAL, 26)))
