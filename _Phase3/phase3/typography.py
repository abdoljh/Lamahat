...
from .typography_common import (
    ...
    SIZES,
    # Typography-over-image overlay renderer.
    render_text_overlay,
)
...
def render_overlay(spec: TypographySpec, out_path: Path) -> Path:
    """
    Render one typography shot as a TRANSPARENT overlay (scrim + text) for the
    typography-over-image path, and return the output path.

    Unlike `render`, this produces an RGBA PNG with a transparent background
    (only a readability scrim + the text are drawn), meant to be composited on
    top of moving footage by render.py.  Family-agnostic: the overlay uses the
    shared light-on-scrim treatment regardless of spec.family, so it reads over
    any background.  The alpha channel is preserved (no flatten/optimize).
    """
    out_path.parent.mkdir(parents=True, exist_ok=True)
    image = render_text_overlay(spec)          # RGBA
    image.save(out_path, format="PNG")
    log.debug("Rendered %s typography overlay → %s",
              spec.template, out_path.name)
    return out_path
