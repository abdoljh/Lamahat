...
def render_parallax_clip(img_bgr: np.ndarray,
    ...
    # v0.3 "inpaint": precompute the amodal layers once per shot.
    if warp == "inpaint":
...
PARALLAX_DEFAULTS: dict[str, dict] = {
    ...
    "archive":  dict(motion="dolly_in", intensity=0.55, amp_px=46, depth_softness=7.0, warp="backward"),
    "location": dict(motion="arc",      intensity=0.85, amp_px=70, depth_softness=6.0, warp="inpaint"),
    "object":   dict(motion="float",    intensity=0.50, amp_px=40, depth_softness=7.0, warp="backward"),
    "broll":    dict(motion="arc",      intensity=0.70, amp_px=60, depth_softness=6.0, warp="inpaint"),
}
_PARALLAX_FALLBACK = dict(motion="dolly_in", intensity=0.50, amp_px=44, depth_softness=7.0, warp="backward")
