...
def camera_path(name: str, t: float, intensity: float = 1.0) -> tuple[float, float, float]:
    ...
    if name == "arc":            # lateral truck L->R with a gentle push — the classic reveal
        return ((2.0 * s - 1.0) * intensity, 0.10 * math.sin(math.pi * t) * intensity,
                1.0 + 0.05 * s * intensity)
    if name == "dolly_in":       # push straight in; near grows faster than far -> depth
        return (0.0, 0.0, 1.0 + 0.10 * s * intensity)
    if name == "dolly_out":
        return (0.0, 0.0, 1.0 + 0.10 * (1.0 - s) * intensity)
    if name == "truck_left":
        return (-1.0 * s * intensity, 0.0, 1.0 + 0.03 * s * intensity)
    if name == "truck_right":
        return (1.0 * s * intensity, 0.0, 1.0 + 0.03 * s * intensity)
    if name == "float":          # slow breathing drift for talking-head / portrait holds
        return (0.18 * math.sin(2 * math.pi * t) * intensity,
                0.12 * math.sin(math.pi * t) * intensity,
                1.0 + 0.02 * s * intensity)
...
def render_parallax_clip(img_bgr: np.ndarray,
    ...
    depth_softness: gaussian sigma to soften depth edges -> hides cardboard-cutout
                seams without forward-warp inpainting (see README v0.2 roadmap).
    """
    ...
    try:
        for f in range(n_frames):
            ...
            # Per-pixel scale: near (disp~1) zooms faster than far -> dolly parallax.
            s = 1.0 + (zoom - 1.0) * disp
            # Backward map: where in the source does each dest pixel sample from?
            #   zoom about principal, then depth-weighted lateral/vertical shift.
            src_x = px + (xs - px) / s + cx * amp_px * disp
            src_y = py + (ys - py) / s + cy * amp_px * disp
            frame_buf = cv2.remap(img, src_x, src_y, interpolation=cv2.INTER_LINEAR,
...
PARALLAX_DEFAULTS: dict[str, dict] = {
    "portrait": dict(motion="dolly_in", intensity=0.40, amp_px=36, depth_softness=8.0),
    "archive":  dict(motion="dolly_in", intensity=0.55, amp_px=46, depth_softness=7.0),
    "location": dict(motion="arc",      intensity=0.85, amp_px=70, depth_softness=6.0),
    "object":   dict(motion="float",    intensity=0.50, amp_px=40, depth_softness=7.0),
    "broll":    dict(motion="arc",      intensity=0.70, amp_px=60, depth_softness=6.0),
}
_PARALLAX_FALLBACK = dict(motion="dolly_in", intensity=0.50, amp_px=44, depth_softness=7.0)
...
def depth_cache_path(image_path) -> "Path":
    """Where the cached depth PNG for an image lives (sibling file)."""
    image_path = Path(image_path)
    return image_path.with_name(image_path.stem + ".depth.png")


def ensure_depth_cached(image_path, *, backend: str = "depthanything") -> "Path":
    """Estimate depth for image_path and cache it next to the image as <stem>.depth.png.

    Call this from prebuild_assets.py once a shot's image is chosen, so the GPU/
    non-deterministic step happens in the asset stage and never in the render loop.
    Returns the depth PNG path. No-op if the cache already exists.
    """
    image_path = Path(image_path)
...
def render_shot_parallax(image_path, clip_path, duration_sec, visual,
    ...
    if depth_png is None:
        depth_png = depth_cache_path(image_path)
    if Path(depth_png).exists():
        depth = load_depth(depth_png)
    else:
        print(f"[motion_parallax] no cached depth for {image_path.name}; "
              f"estimating at render time (prefer prebuild_assets caching).")
        ensure_depth_cached(image_path, backend=backend)
        depth = load_depth(depth_cache_path(image_path))
    img = cv2.imread(str(image_path), cv2.IMREAD_COLOR)
