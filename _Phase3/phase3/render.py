    ...
    if fetcher is not None:
        ...
        if result and result.has_image:
            ...
            try:
                ...
                # If prebuild_assets cached a depth map next to the source
                # image (<stem>.depth.png), carry it alongside the asset so
                # parallax can skip re-estimating depth at render time.
                try:
                    src_depth = Path(best.local_path).with_name(
                        Path(best.local_path).stem + ".depth.png")
                    if src_depth.exists():
                        shutil.copyfile(
                            src_depth,
